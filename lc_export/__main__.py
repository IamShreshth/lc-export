...
from lc_export.utils import VALID_PROGRAMMING_LANGUAGES
...
def main():
    ...
    logging.info("lc-export run with arguments: " + str(args))
    ...
    for submission in leetcode.get_submissions():
        if (
            ...
            logging.warning(
                "Submissions are not in reverse chronological order, --only-last-submission flag might not work as expected if used."
            )
